        ...
        with st.sidebar:
            ...
            if dati_filtrati:
                ...
                with xlsxwriter.Workbook(output) as wb:
                    ...
                    ws.write_row(0, 0, ["Cassa", "Codice", "Cliente", "Data", "Livello", "Pezzi"])
                    
                    last_session = None
                    for r, entry in enumerate(dati_filtrati, 1):
                        # Se la sessione è la stessa della riga precedente, lascia vuoto
                        if entry['session_id'] == last_session:
                            ws.write(r, 4, entry['Livello'])
                            ws.write(r, 5, entry['Pezzi'])
                        else:
                            ws.write_row(r, 0, [entry['Cassa'], entry['Codice'], entry['Cliente'], entry['Data'], entry['Livello'], entry['Pezzi']])
